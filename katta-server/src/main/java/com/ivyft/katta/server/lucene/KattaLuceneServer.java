...
public class KattaLuceneServer implements IContentServer, KattaServerProtocol {
     ...
     *
     */
    protected KattaSerialSocketServer serialSocketServer;
    ...
    public KattaLuceneServer() {
        LuceneServer.DEFAULT_QUERY.setStart(0);
        LuceneServer.DEFAULT_QUERY.setRows(10);
        LuceneServer.DEFAULT_QUERY.set(CommonParams.TIME_ALLOWED, 5 * 1000);
    }
    ...
    public void init(String nodeName, NodeConfiguration nodeConfiguration) {
        ...
        this.seacherFactory = (ISeacherFactory) ClassUtil.newInstance(
                ...
                LuceneServer.CONF_KEY_SEARCHER_FACTORY_CLASS, DefaultSearcherFactory.class));


        this.closeIndexSearcherPolicy = (CloseIndexSearcherPolicy) ClassUtil.newInstance(
                nodeConfiguration.getClass(
                        LuceneServer.CONF_KEY_SEARCHER_CLOSE_POLICY_CLASS, DefaultCloseIndexSearcherPolicy.class));
        this.closeIndexSearcherPolicy.init(nodeConfiguration);

        this.timeoutPercentage = nodeConfiguration.getFloat(LuceneServer.CONF_KEY_COLLECTOR_TIMOUT_PERCENTAGE, this.timeoutPercentage);
        ...
        if (this.timeoutPercentage < 0 || this.timeoutPercentage > 1) {
            throw new IllegalArgumentException("illegal value '" + this.timeoutPercentage + "' for "
                    + LuceneServer.CONF_KEY_COLLECTOR_TIMOUT_PERCENTAGE + ". Only values between 0 and 1 are allowed.");
        }
        int coreSize = nodeConfiguration.getInt(LuceneServer.CONF_KEY_SEARCHER_THREADPOOL_CORESIZE, 10);
        int maxSize = nodeConfiguration.getInt(LuceneServer.CONF_KEY_SEARCHER_THREADPOOL_MAXSIZE, coreSize * 8);
        this.facetCacheMB = nodeConfiguration.getDouble(LuceneServer.FACET_CACHE_MB, 64.0D);
        ...
        int filterCacheSize = nodeConfiguration.getInt(LuceneServer.CACHE_MAX_NUMBER_SIZE, 5000);
        int filterCacheTimeMinutes = nodeConfiguration.getInt(LuceneServer.CACHE_MAX_EXPIRE_MINUTES, 30);
        //初始化缓存
        ...
        //初始化Hadoop Adapter
        serialSocketServer = new KattaSerialSocketServer(this, nodeConfiguration);
    ...
    public void removeShard(final String shardName) {
        LOG.info("KattaLuceneServer " + this.nodeName + " removing shard " + shardName);
        ...
        if (handle != null) {
            try {
                ...
                LOG.error("KattaLuceneServer " + this.nodeName + " error removing shard " + shardName, e);
            }
        }
